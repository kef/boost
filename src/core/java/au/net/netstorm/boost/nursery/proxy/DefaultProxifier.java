...
public final class DefaultProxifier implements Proxifier {
    ...
    public <T> T closure(T ref, ProxySpec spec) {
        Class<Closure>[] closures = spec.get();
        return (T) enclose(ref, closures);
    }

    public <T> T closure(T ref, Class<? extends Closure>... closures) {
        return (T) enclose(ref, closures);
    }

    private Object enclose(Object ref, Class<? extends Closure>... classes) {
        Object closed = ref;
    ...
    private Object proxy(Object ref, Class<? extends Closure> cls) {
        Interface[] types = ifaces(ref);
        Closure closure = nu.nu(cls, ref);
